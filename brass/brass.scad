...
// Part box dimensions
PART_BOX_LENGTH  =   45 * mm;
IRON_COAL_HEIGHT = 0.75 * inch;
...
module tile_box( tiles ) {
    ...
    iz = COMPARTMENT_HEIGHT/2 - TOP - BOTTOM; // layer_height( tile_height( max( tiles ) ) );
...
module part_box( mode=0 ) {
    ix = PART_BOX_LENGTH - 4*THIN_WALL - 2*GAP;
    iy = COMPARTMENT_DEPTH - 4*THIN_WALL - 2*GAP;
    iz = COMPARTMENT_HEIGHT/2 - TOP - BOTTOM;
...
module part_lid() {
    ix = PART_BOX_LENGTH - 2*THIN_WALL - 2*GAP;
    iz = COMPARTMENT_HEIGHT/2 - TOP - BOTTOM;
...
if (PART == "lancashire-tile-box") {
    ...
    translate( [  0, 90, 0] ) part_lid();
    
    translate( [ 50,  0, 0] ) part_box( BIRMINGHAM );
    translate( [ 50, 90, 0] ) part_lid();

//     translate( [100,  0, 0] ) part_box( LANCASHIRE );
//     translate( [100, 90, 0] ) part_lid();
}